...
def get_config_filtered_by_active_service(scfg):
    if not isinstance(scfg, dict):
        com.log_error("Configuration error - Config must be a dictionary - type: {} / content: {}".
                      format(type(scfg), scfg))

    active_services = {}
    config_data = {}

    client_name = ""
    for first_item in scfg.keys():
        client_name = first_item
        config_data = scfg[first_item]
    ...
    return client_name, active_services
...
def parse_parameters_and_values_from_config(config_data):
    ...
    client_name, scfg = get_config_filtered_by_active_service(config_data)
